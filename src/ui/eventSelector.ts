...
export function clearEventSelectorCards(state: AppState): void {
...
export function moveEventSelectorSelection(widgets: Widgets, state: AppState, nextIndex: number): void {
...
export function renderEventSelector(widgets: Widgets, state: AppState): void {
...
export function selectorStatusLabel(status: string): string {
  ...
  if (text === 'in' || text === 'live') return 'LIVE';
  if (text === 'pre') return 'UP NEXT';
  if (text === 'post') return 'FINAL';
  return (text || '--').toUpperCase();
}

function updateEventSelectorCard(widgets: Widgets, state: AppState, index: number, layout?: any): void {
...
function getEventSelectorLayout(widgets: Widgets): any {
...
function syncEventSelectorScroll(widgets: Widgets, state: AppState): void {
  if (!widgets.eventSelectorBox || !state.eventSelectorOptions.length) return;
